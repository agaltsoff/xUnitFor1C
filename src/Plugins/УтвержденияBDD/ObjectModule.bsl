...
Функция СуществуетВИнформационнойБазе() Экспорт
	Если ТипЗнч(ПроверяемоеЗначение) <> Тип("Строка") Тогда
		Если Не ЛогическоеВыражениеВерно(
				ПроверяемоеЗначение <> Неопределено 
				И ПроверяемоеЗначение <> Null 
				И Найти(Строка(ПроверяемоеЗначение), "<Объект не найден>") = 0
			) 
		Тогда
			СообщениеОшибки = СформироватьСообщениеОбОшибке(ПроверяемоеЗначение, "СУЩЕСТВУЕТ В ИНФОРМАЦИОННОЙ БАЗЕ.");
			ВызватьОшибкуПроверки(СообщениеОшибки);
		КонецЕсли;
	КонецЕсли;
...
Функция Пустой() Экспорт
	...
	Перем Пустой;
	Перем ПроверяемоеЗначениеПредставление;
	
	Если ТипЗнч(ПроверяемоеЗначение) = Тип("РезультатЗапроса") Тогда
		Пустой= ПроверяемоеЗначение.Пустой();
		ПроверяемоеЗначениеПредставление= "Результат запроса";
	Иначе
		Попытка
			Пустой = Не ПроверяемоеЗначение.Количество();
		Исключение
			Пустой = Не ЗначениеЗаполнено(ПроверяемоеЗначение);
		КонецПопытки;
		ПроверяемоеЗначениеПредставление= ПроверяемоеЗначение;
	КонецЕсли;
	
	Если Не ЛогическоеВыражениеВерно(Пустой) Тогда
		СообщениеОшибки = СформироватьСообщениеОбОшибке(ПроверяемоеЗначениеПредставление, "ПУСТОЙ.");
		ВызватьОшибкуПроверки(СообщениеОшибки);
	КонецЕсли;
