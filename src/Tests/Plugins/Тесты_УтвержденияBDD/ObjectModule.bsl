...
Функция ПолучитьСписокТестов() Экспорт
	...
	ВсеТесты.Добавить("ТестДолжен_Проверить_СуществуетВИнформационнойБазе");
	...
	ВсеТесты.Добавить("ТестДолжен_Проверить_Пустой");
...
Процедура ТестДолжен_Проверить_СуществуетВИнформационнойБазе() Экспорт
	
	// Простые типы существуют
	
	Ожидаем.Что(0).СуществуетВИнформационнойБазе();
	Ожидаем.Что("").СуществуетВИнформационнойБазе();
	Ожидаем.Что(Новый Массив).СуществуетВИнформационнойБазе();
	
	Элемент= Справочники.ПростойСправочник.СоздатьЭлемент();
	
	Элемент.Записать();
	
	Ссылка= Элемент.Ссылка;
	
	// Созданный элемент элемент существует
	
	Ожидаем.Что(Ссылка).СуществуетВИнформационнойБазе();
	
	Элемент.Удалить();
	
	// Удалённый элемент не существует
	
	Ожидаем.Что(Ссылка).Не_().СуществуетВИнформационнойБазе();
	
КонецПроцедуры

Процедура ТестДолжен_Проверить_Пустой() Экспорт
	
	Ожидаем.Что(0).Пустой();
	Ожидаем.Что("").Пустой();
	
	Ожидаем.Что(1).Не_().Пустой();
	Ожидаем.Что("1").Не_().Пустой();
	
	Массив= Новый Массив;
	
	Ожидаем.Что(Массив).Пустой();
	
	Массив.Добавить();
	
	Ожидаем.Что(Массив).Не_().Пустой();
	
	Запрос = Новый Запрос;
	
	Запрос.Текст = "ВЫБРАТЬ ИСТИНА ГДЕ ЛОЖЬ";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Ожидаем.Что(РезультатЗапроса).Пустой();
	
	Запрос.Текст = "ВЫБРАТЬ ИСТИНА";
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Ожидаем.Что(РезультатЗапроса).Не_().Пустой();
